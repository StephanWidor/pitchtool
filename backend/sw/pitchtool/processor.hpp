...
private:
    void filterSpectrum(ChannelState<F> &io_state)    // swdebug: binSpectrum and spectrum as direct params
    {
    ...
    void processChannel(const ChannelParameters<F> &parameters, const TuningParameters<F> &tuningParameters,
                        const F timeDiff, const int stepSize, ChannelState<F> &io_state)
    {
        ...
        const auto pitchFactor =
          tuningFactor(tuningParameters, parameters.tuningType, io_state.tuningEnvelope, timeDiff) *
    ...
    F tuningFactor(const TuningParameters<F> &tuningParameters, const tuning::Type &type,
                   TuningNoteEnvelope<F> &tuningEnvelope, const F timeDiff)
    {
    ...
    void alignFormants(const std::vector<SpectrumValue<F>> &spectrum, std::vector<SpectrumValue<F>> &io_spectrumToAlign)
    {
